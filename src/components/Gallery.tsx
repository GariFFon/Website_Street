      ...
      <div className="container mx-auto px-4">
        <div className="text-center mb-16">
          <h2 className="font-display text-6xl md:text-7xl mb-4">
            ONE OF <span className="text-modkicks-red">ONE</span> GALLERY
          </h2>
          <p className="text-lg max-w-2xl mx-auto text-gray-400">
            Explore our showcase of past custom projects. Each piece is unique
            and one-of-a-kind, never to be replicated again.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16">
          {galleryItems.map((item, index) => (
            <div
              key={index}
              className="group relative overflow-hidden rounded-2xl aspect-square cursor-pointer shadow-lg hover:shadow-2xl hover:shadow-modkicks-red/20 transition-all duration-500"
            >
              <img
                src={item.image}
                alt={item.title}
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/50 to-transparent flex flex-col justify-end p-6">
                <div className="transform translate-y-6 group-hover:translate-y-0 transition-transform duration-500">
                  <h3 className="font-bold text-xl mb-2 text-white group-hover:text-modkicks-red transition-colors">
                    {item.title}
                  </h3>
                  ...
                  <div className="flex items-center justify-between opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-200">
                    <span className="text-xs text-modkicks-red border border-modkicks-red/40 px-3 py-1 rounded-full">
                      By {item.artist}
                    </span>
                    <button className="text-white hover:text-modkicks-red transition-colors">
                      <svg
        ...
        <div className="text-center">
          <Button
            variant="outline"
            size="lg"
            className="border-white text-white hover:bg-white hover:text-modkicks-black"
          >
            View Full Gallery
          </Button>
        </div>
